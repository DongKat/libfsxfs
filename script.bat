@echo off
REM filepath: d:\OPSWAT\Workspace\ARC\ARC\CLIExtractionSchema\xfs.bat
SET fsxfsmountPath="%~dp0\fsxfsmount.exe"
SET xfsFilePath=%1
SET outputPath=%2
SET UNIQUE_ID=xfs_mount_%RANDOM%
SET mountPoint=%TEMP%\%UNIQUE_ID%

REM Create mount point directory
mkdir "%mountPoint%"
echo Mounting XFS filesystem from %xfsFilePath% to %mountPoint%

REM Mount the XFS filesystem in background using a unique window title
start "%UNIQUE_ID%" %fsxfsmountPath% "%xfsFilePath%" "%mountPoint%"
...
REM Unmount by killing fsxfsmount using its PID
